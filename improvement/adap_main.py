...
def main(args):
    for i in range(10):
        writer = SummaryWriter('./adap_log2/gaussain_HIGH_flodding_adap_'+str(i))
        ...
        while loss > 1e-04 :
            ...
            writer.add_scalar("Loss/tr_acc", tr_acc, cnt)
            writer.add_scalar("Loss/tr_loss", tr_loss, cnt)
            writer.add_scalar("Loss/va_acc", va_acc, cnt)
            #writer.add_scalar("Loss/proportion", proportion, cnt)

            writer.add_scalar("Loss/va_loss", va_loss, cnt)
            writer.add_scalar("Loss/te_acc", te_acc, cnt)
            writer.add_scalar("Loss/te_loss", te_loss, cnt)

            writer.add_scalar("Loss/tr_cl_acc", tr_cl_acc, cnt)
            writer.add_scalar("Loss/tr_cl_loss", tr_cl_loss, cnt)
            writer.add_scalar("Loss/va_cl_acc", va_cl_acc, cnt)

            writer.add_scalar("Loss/va_cl_loss", va_cl_loss, cnt)
            writer.add_scalar("Loss/te_cl_acc", te_cl_acc, cnt)
            writer.add_scalar("Loss/te_cl_loss", te_cl_loss, cnt)


            print('###### flood: {} rs: {}####'.format(0, args.random_seed))
            print('Epoch: {} LR: {} TrLss: {:.4g} VaLss: {:.4g} TeLss: {:.4g} TrAcc: {:.3g} VaAcc: {:.3g} TeAcc: {:.3g}'.format(
                cnt+1, current_lr, tr_loss, va_loss, te_loss, tr_acc, va_acc, te_acc))
            ...
            init_flood_level = 0.0001
            init_val_acc = 0.0
        for epoch in range(cnt, args.epochs):
            ...
            if epoch != -1: # skip epoch 0 for training
                for (images, labels, _) in train_loader:
                    ...
                    loss_corrected = (loss_mean-init_flood_level).abs() + init_flood_level
                    mini_batch_count += 1
                    optimizer.zero_grad()
            ...
            writer.add_scalar("Loss/flood_level", init_flood_level, epoch)


            print('###### flood: {} rs: {}####'.format(init_flood_level, args.random_seed))
            print('Epoch: {} LR: {} TrLss: {:.4g} VaLss: {:.4g} TeLss: {:.4g} TrAcc: {:.3g} VaAcc: {:.3g} TeAcc: {:.3g}'.format(
            ...
            if va_acc >= init_val_acc+0.4 :
                print("Here 111")
                init_val_acc = va_acc 
                flood_level =  np.clip(init_flood_level+0.015,0.0001,0.5)
                init_flood_level =flood_level
            ...
            print(f'currnet _vacc :{va_acc}, init_val_acc:{init_val_acc}, value+0.05 :{init_val_acc+0.05}, value=1.05:{init_val_acc-0.05}')
